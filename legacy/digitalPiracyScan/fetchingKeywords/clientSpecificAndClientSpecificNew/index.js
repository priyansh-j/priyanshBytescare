...
exports.handler = async (event, context, callback) => {
    ...
    if (event.queryStringParameters.engine == "google" && event.queryStringParameters.key.search(/^https?(:|%3A)/gi) > -1) {
    ...
    await fetch.fetch(rid, date_init, date_final, crawler_list, event.queryStringParameters.env, decrypted, function (err, results) {
        if (err) throw err;
        result = results;
    ...
    S3.putObject({
        ...
        Key: "data-digital-piracy-scan/client-data.json",
        Body: `${result}`,
    })
        .promise()
