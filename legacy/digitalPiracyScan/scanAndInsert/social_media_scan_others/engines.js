...
function filterBooks(searchTitle, bookListings, similarityThreshold = 0) { //keeping threshold 0 as only score calculation is needed
...
async function extract (bucket_path, aid,env,decrypted) {

    
    let bucket =  'credentials-db';
    ...
    let fetch_results=   await set.set(results.body.results, 2, aid,env,decrypted) ;
    //console.log(fetch_results);
   // process.exit();
    for (var len=fetch_results.length - 1 ; len >=0; len --)
 
    {  const searchTitle = fetch_results[len]['title'].toLowerCase().trim();
   ...
   // return fetch_results
    insert.insert( fetch_results , aid, env, decrypted, function(result){
