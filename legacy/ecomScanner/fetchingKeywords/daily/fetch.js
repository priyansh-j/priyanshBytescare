...
async function fetch(date_init,date_final,crawler_list, env, decrypted,callback){
 if (env == "dev"){    
      ...
      {    
            ...
            try{
                if (results[count].cycle == 241 && (results[count].expiry > date ) && ((results[count].type == 'movie' || results[count].type == 'brand_content' || results[count].type == 'brand_trademark'|| results[count].type == 'book' || results[count].type == 'brand' )))
                                {
