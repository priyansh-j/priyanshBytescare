...
async function insert(el, asset_id, env, decrypted, engine , tid,callback){
    ...
    if (env == 'dev'){
        ...
        for (let i = 0; i < el.length; i++) {
            ...
            arr.push([el[i].pid, el[i].title, el[i].source, "NA",el.type,engine, date, date,el[i].score, el[i].prediction,0,null,subdomain,domain]) 
            ;}
        var con = await mysql.createConnection({
          ...
          if (err) throw err;
            ...
            await con.query('insert ignore into social_search values ?', [arr], async function (err, result, fields) {
              if (err) {
                throw err;
              }
              ...
              for (let i = 0; i < el.length; i++) {
                map_array.push([el[i].pid, asset_id, date, "asset_social_search"]);
                map_array2.push([el[i].pid, tid, date, "social_search"]);
              }
        ...
        for (let i = 0; i < el.length; i++) {
             arr.push([el[i].pid, el[i].title, el[i].source, "NA",el.type,engine, date, date,el[i].score, el[i].prediction,0,null])}
        var con = await mysql.createConnection({
            host: process.env.db_host_dev,
            user: process.env.database_username_dev,
            password: decrypted,
